#!/usr/bin/env python
"""
scripts/run_prithvi_fcn_experiments.py

Orchestrates batch submissions of Prithvi FCN adaptation experiments on the SLURM cluster.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Sweep Prithvi FCN experiments on SLURM.")
    parser.add_argument(
        "--budgets",
        type=int,
        nargs="+",
        default=[1, 10, 25, 100],
        help="Data budgets to sweep (1, 10, 25, 100)"
    )
    parser.add_argument(
        "--models",
        type=str,
...
def main():
    ...
    print("=" * 60)
    print("      PRITHVI FCN EXPERIMENT SWEEP ORCHESTRATOR")
    print("=" * 60)
    ...
    for budget in args.budgets:
        for model in args.models:
            for seed in args.seeds:
                ...
                run_name = f"{model}_seed_{seed}_split_{budget_str}"
                ...
                if metrics_file.exists():
                    print(f"Skipping: {model} | Seed {seed} | Budget {budget}% (already completed)")
                    print("-" * 50)
                    continue
...
"""
                ...
                # 4. Submit the job
                print(f"Queueing: {model} | Seed {seed} | Budget {budget}%")
